        ...
        <h-row>
          <h-col span="6">
            <h-form-item label="交易状态：" prop="state">
              <h-select v-model="FormData.state" placeholder="已确认">
                <h-option
...
export default {
  data() {
    ...
    return {
      ...
      transColumns: [
        ...
        //     if (value === 1) {
        //       return row.userType == "机构";
        //     } else if (value === 2) {
        //       return row.userType == "个人";
        //     }
        //   },
        //   render: (h, { row: { userType } }) => {
        //     return h("span", {}, this.userType);
        //   },
        // },
        {
          title: "产品代码",
    ...
    transDetail() {
      if (this.detailData.state === 1) {
        return [
          ...
          {
            item: this.detailData.type == "申购" ? `计算后份额` : `计算后金额`,
            value:
              this.detailData.type == "申购"
                ? this.detailData.count
                : this.detailData.amount,
  ...
  methods: {
    ...
    getTransList() {
      ...
      if (this.FormData.state === 1 || this.FormData.state === undefined) {
        this.getChecked();
      } else {
        this.getUnchecked();
      }
    ...
    getChecked() {
      core
        .fetch({
          method: "post",
          url: `/apiPtrans/purchase/findByAll`,
          data: {
            ...this.FormData,
          },
        })
      ...
      core
        .fetch({
          method: "post",
          url: `/apiRtrans/redemption/findByAll`,
          data: {
            ...this.FormData,
          },
        })
    ...
    getDetail(state, row) {
      ...
      //     if (code === 1) {
      //       //data中的user_id截取10位
      //       data.user_id = data.user_id.slice(-10);
